...
get_claude_project_dir_name <- function() {
  wd <- getwd()
  if (grepl("^[A-Za-z]:[/\\\\]", wd)) {
    # Windows: "C:<barra>Users<barra><usuário>..." -> "c--Users-<usuário>..."
    drive <- tolower(substr(wd, 1, 1))
    rest <- substr(wd, 4, nchar(wd))
    rest <- gsub("[/\\\\]", "-", rest)
    paste0(drive, "--", rest)
  } else {
    # macOS/Linux: "<barra>Users<barra><usuário><barra>Meu Projeto" ->
    # "-Users-<usuário>-Meu-Projeto" (Claude Code substitui todo caractere não
    # alfanumérico, incluindo "_" e espaço, por "-" ao nomear a pasta de
